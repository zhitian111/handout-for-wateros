...
#### 多路重定向
重定向符号可以同时使用多个，但shell会遵循后发先至的原则，使用最后一次定向的文件。
```bash
...
./myprogram > 1.txt > 2.txt  # 输出重定向到2.txt
```
#### 管道
...
- 管道
- 套接字
系统内部通过文件描述符（file descriptor）统一管理这些资源。
这也是为什么：
```
stdout 可以重定向到文件
```
...
例如：
```bash
cargo build && echo "Build success"
```
等价于：
```bash
if cargo build; then  
...
可以组合多个条件：
```bash
if cargo build && qemu-system-riscv64 ...; then  
    echo "All success"  
else  
...
echo "Running QEMU..."
qemu-system-riscv64 -kernel target/debug/kernel
```
## 判断是否存在工具
```bash
